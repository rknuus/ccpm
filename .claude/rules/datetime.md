...
When creating or updating frontmatter in any file (Initiative, Epic, Task, Progress), always use the real current datetime:
...
2. **For commands that create files:**
   - Initiative creation: Use real date for `created` field
...
Commands affected: initiative-new, initiative-parse, epic-decompose, epic-sync, issue-start, issue-sync, and any other command that writes timestamps.